...
for main_label in ['Long',]:
    ...
    sp_tdc_meas = subplot(sp_ctr_p, title='Current profiles', xlabel='time [fs]', ylabel='Current (kA)')
    sp_ctr_p += 1

    sp_backtrack_tdc_screen = subplot(sp_ctr_p, title='Screen projections', xlabel='x [mm]', ylabel='Screen projection')
    sp_ctr_p += 1

    #sp_back_forward = subplot(sp_ctr_p, title='Reconstruction forward', xlabel='x [mm]', ylabel='Screen projection')
    sp_ctr_p += 1

    sp_recon = subplot(sp_ctr_p, title='Optimized current profiles', xlabel='time [fs]', ylabel='Current (kA)')
    sp_ctr_p += 1
    ...
    for sp_ in sp_tdc_meas, sp_recon:
        profile_meas.plot_standard(sp_, center=center, label='TDC' % (profile_meas.gaussfit.sigma*1e15), color='black')
    ...
    for sp_ in sp_backtrack_tdc_screen, sp_recon_screen: #sp_back_forward:
        median_screen.plot_standard(sp_, label='Measured', color='black')

    median_screen_sigma = median_screen.gaussfit.sigma

    all_screens = {'median_screen': median_screen}

    for q_wake, label in [(False, 'Dipole'), (True, '+Quadrupole')][:1]:
        ...
        profile_tdc_back.plot_standard(sp_tdc_meas, center=center, label='Backpropagated')
        ...
        color = screen_forward1.plot_standard(sp_backtrack_tdc_screen, label='TDC forward')[0].get_color()
        ...
        label2 = label+' %i' % (bp_recon.gaussfit.sigma*1e15)
        label2 = label
        bp_recon.plot_standard(sp_recon, label=label2, center=center)
