...
sp_res = subplot(sp_ctr, title='Resolution', xlabel='t (fs)', ylabel='Resolution (fs)')
sp_ctr += 1
...
for ctr, (streaker_calib_file, one_sided) in enumerate(all_streaker_calib):
    ...
    calib_dict = sc.analyze_streaker_calibration(input_dict, do_plot=True, fit_order=True, fit_gap=False, forward_propagate_blmeas=True, tracker=tracker, blmeas=blmeas_file)
    ...
    gap_fit = meta_data['semigap']*2
    order_fit = meta_data['fit_dict_centroid']['order_fit']
    ...
    tt, res = iap.calc_resolution(blmeas_profile, gap_fit, offsets[0], 1, tracker, 1)
